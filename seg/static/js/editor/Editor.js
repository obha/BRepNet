...
import { getRecursiveRootNode } from "../utils/three.js";

var _DEFAULT_CAMERA = new THREE.PerspectiveCamera(
  75,
...
class Editor {
  ...
  addObjectClickEvent(object, intersectionHandler) {
    ...
    const handler = (event) => {
      ...
      if (object) {
        const intersection = raycaster
          ...
          .map((node) => {
            node.object = getRecursiveRootNode(this.scene, node.object);
            return node;
          })
